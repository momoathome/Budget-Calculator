...
<template>
  <li class="cashlist-item text-primary">
    <form action="" class="flex" @submit.prevent="onSubmit">
      <input
        v-model="inputValue" type="text" :placeholder="`new ${description}`"
        class="me-1 w-full border-none bg-base py-3 ps-4 text-(xl base_dark) font-600 outline-2 outline-primary hover:(cursor-pointer outline-solid) focus-visible:(cursor-text outline-solid placeholder-text-primary_light) placeholder:(font-600 text-primary)"
      >
      <div class="pointer-events-none bg-base">
        <span class="mt-6px inline-block h-75% w-2px bg-list_divider" />
      </div>
      <input
        v-model="inputAmount" type="number" placeholder="amount" min="0" step="0.01"
